...
# Grid
GRID_SIZE = 16
GRID_COLOR = "#444444" # Darker grid for dark theme
GRID_MAJOR_COLOR = "#666666"
...
# Colors (Clickteam Fusion 2.5 Dark Skin Palette)
COL_BG = "#2D2D30"       # Main Background
COL_BG_LIGHT = "#3E3E42" # Lighter elements
COL_PANEL = "#252526"    # Panels (Properties, Workspace)
COL_PANEL_HEADER = "#333337"
COL_ACCENT = "#007ACC"   # Fusion/VS Blue Selection
COL_ACCENT_HOVER = "#1C97EA"
COL_TEXT = "#F1F1F1"     # Main Text
COL_TEXT_DIM = "#999999"
COL_TOOLBAR = "#2D2D30"
COL_CANVAS_BG = "#1E1E1E" # Canvas Dark
COL_SELECTION = "#007ACC"
COL_DANGER = "#E51400"
COL_SUCCESS = "#107C10"
COL_WARNING = "#FFC83D"

# Fonts (Standard Windows Application Look)
FONT_MAIN = ("Segoe UI", 9)
FONT_BOLD = ("Segoe UI", 9, "bold")
FONT_SMALL = ("Segoe UI", 8)
FONT_MONO = ("Consolas", 10)

# Custom Button Styles (Fusion Style)
COL_BTN_FACE = "#3E3E42"  
COL_BTN_TEXT = "#F1F1F1"
COL_BLUE_TEXT = "#569CD6" # Visual Studio Blue Text
...
class ExpressionBuilder(tk.Toplevel):
    def __init__(self, master, callback=None):
        ...
        tk.Label(self, text="Build Expression", bg=COL_BG, fg=COL_TEXT,
                font=FONT_BOLD).pack(pady=5)
          
        # Expression entry
        self.expr_var = tk.StringVar()
        entry = tk.Entry(self, textvariable=self.expr_var, font=FONT_MONO,
        ...
        for i, expr in enumerate(EXPRESSIONS):
            btn = tk.Button(btn_frame, text=expr, bg=COL_BTN_FACE, fg=COL_BTN_TEXT,
                          ...
                          relief=tk.FLAT, padx=5, pady=2, font=FONT_MAIN)
        ...
        tk.Button(bottom, text="OK", bg=COL_ACCENT, fg="white", font=FONT_MAIN,
                 command=self.ok).pack(side=tk.RIGHT, padx=5)
        tk.Button(bottom, text="Cancel", bg=COL_BTN_FACE, fg=COL_BTN_TEXT, font=FONT_MAIN,
...
class AnimationEditor(tk.Toplevel):
    def __init__(self, master, obj_data, callback=None):
        ...
        tk.Label(left, text="Animations", bg=COL_PANEL_HEADER, fg=COL_TEXT,
                font=FONT_BOLD).pack(fill=tk.X)
          
        self.anim_listbox = tk.Listbox(left, bg=COL_BG_LIGHT, fg=COL_TEXT, font=FONT_MAIN,
        ...
        self.frames_entry = tk.Entry(right, textvariable=self.frames_var,
                                    font=FONT_MONO, bg=COL_BG_LIGHT, fg=COL_TEXT,
                                    insertbackground=COL_TEXT)
        self.frames_entry.pack(fill=tk.X, padx=10)
          
        tk.Label(right, text="Speed (ms per frame):", bg=COL_BG, fg=COL_TEXT, font=FONT_MAIN).pack(pady=5)
        ...
        # Preview area
        tk.Label(right, text="Preview:", bg=COL_BG, fg=COL_TEXT, font=FONT_MAIN).pack(pady=5)
        ...
        tk.Button(bottom, text="Save", bg=COL_ACCENT, fg="white", font=FONT_MAIN,
                 command=self.save).pack(side=tk.RIGHT, padx=5)
        tk.Button(bottom, text="Close", bg=COL_BTN_FACE, fg=COL_BTN_TEXT, font=FONT_MAIN,
                 command=self.destroy).pack(side=tk.RIGHT, padx=5)
...
class EventEditorPanel(tk.Frame):
    ...
    def setup_ui(self):
        ...
        tk.Button(toolbar, text="+ New Event", bg=COL_SUCCESS, fg="white", font=FONT_MAIN,
                 relief=tk.FLAT, command=self.add_event_group).pack(side=tk.LEFT, padx=5)
        tk.Button(toolbar, text="+ Comment", bg=COL_WARNING, fg="black", font=FONT_MAIN,
                 relief=tk.FLAT, command=self.add_comment).pack(side=tk.LEFT, padx=5)
        tk.Button(toolbar, text="Delete Selected", bg=COL_DANGER, fg="white", font=FONT_MAIN,
    ...
    def refresh(self):
        ...
        if not events:
            tk.Label(self.inner_frame, text="No events yet. Click '+ New Event' to start programming!",
                    bg=COL_BG, fg=COL_TEXT_DIM, font=FONT_MAIN).pack(pady=20)
            return
        ...
        tk.Label(header, text="Conditions", bg=COL_PANEL_HEADER, fg=COL_TEXT, width=40,
                font=FONT_BOLD, anchor="w").pack(side=tk.LEFT, padx=5)
        ...
        for i, group in enumerate(events):
            ...
            if group.get("comment"):
                tk.Label(row, text=f"  💬 {group['comment']}", bg="#4a4a2a", fg="#dddd88",
                        font=(FONT_MAIN[0], FONT_MAIN[1], "italic"), anchor="w").pack(fill=tk.X)
                continue
            ...
            cond_label = tk.Label(row, text=cond_text, bg=row_bg, fg="#7fbaff" if conds else COL_TEXT_DIM,
                                 width=40, anchor="w", font=FONT_MAIN)
    ...
    def pick_condition_action(self, group, frame):
        ...
        cond_listbox = tk.Listbox(cond_frame, bg=COL_BG_LIGHT, fg=COL_TEXT, font=FONT_MAIN,
                                 selectbackground=COL_ACCENT, borderwidth=0)
        ...
        tk.Entry(cond_target_frame, textvariable=cond_target_var, bg=COL_BG_LIGHT, fg=COL_TEXT,
                insertbackground=COL_TEXT, font=FONT_MAIN).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ...
        tk.Checkbutton(cond_frame, text="Negate (NOT)", variable=cond_negate_var,
                      ...
                      activebackground=COL_PANEL, font=FONT_MAIN).pack(anchor="w", padx=5)
        ...
        tk.Button(cond_frame, text="+ Add Condition", bg=COL_ACCENT, fg="white", font=FONT_MAIN,
        ...
        act_listbox = tk.Listbox(act_frame, bg=COL_BG_LIGHT, fg=COL_TEXT, font=FONT_MAIN,
                                selectbackground=COL_ACCENT, borderwidth=0)
        ...
        tk.Button(act_frame, text="+ Add Action", bg=COL_SUCCESS, fg="white", font=FONT_MAIN,
        ...
        tk.Button(bottom, text="✓ Add Event", bg=COL_ACCENT, fg="white", font=FONT_MAIN,
    ...
    def pick_condition_action_edit(self, group):
        ...
        tk.Label(dlg, text="Current Conditions:", bg=COL_BG, fg=COL_ACCENT,
                font=FONT_BOLD).pack(anchor="w", padx=10, pady=5)
        ...
        for i, c in enumerate(group.get("conditions", [])):
            ...
            tk.Label(row, text=f"  {neg}{c['type']}{tgt}", bg=COL_PANEL, fg="#7fbaff",
                    font=FONT_MAIN, anchor="w").pack(side=tk.LEFT, fill=tk.X, expand=True)
        ...
        tk.Label(dlg, text="Current Actions:", bg=COL_BG, fg=COL_SUCCESS,
                font=FONT_BOLD).pack(anchor="w", padx=10, pady=(10, 5))
        ...
        tk.Button(dlg, text="Close", bg=COL_BTN_FACE, fg=COL_BTN_TEXT, font=FONT_MAIN,
...
class LayerManager(tk.Frame):
    def __init__(self, master, frame_data, on_change=None):
        ...
        # Blue text for + and - as requested
        tk.Button(btn_row, text="+", width=3, bg=COL_BTN_FACE, fg=COL_BLUE_TEXT, font=FONT_BOLD,
        ...
        tk.Button(btn_row, text="↑", width=3, bg=COL_BTN_FACE, fg=COL_BTN_TEXT, font=FONT_BOLD,
    ...
    def refresh(self):
        ...
        for i, layer in enumerate(layers):
            ...
            vis_text = "👁" if layer.get("visible", True) else "  "
...
class ACEngine:
    ...
    def setup_toolbar(self):
        ...
        for name, icon, tooltip in tabs:
            btn = tk.Button(tab_frame, text=icon, bg=COL_BTN_FACE, fg=COL_BTN_TEXT,
                          relief=tk.FLAT, padx=10, pady=2, font=("Segoe UI", 12),
        ...
        # Right side: Run buttons (Icons)
        tk.Button(self.toolbar, text="▶", bg=COL_BTN_FACE, fg=COL_SUCCESS,
                 font=("Arial", 12, "bold"), relief=tk.FLAT, padx=10,
                 command=self.run_game).pack(side=tk.RIGHT, padx=5)
        tk.Label(self.toolbar, text="App", bg=COL_TOOLBAR, fg=COL_TEXT_DIM, font=FONT_SMALL).pack(side=tk.RIGHT)

        tk.Button(self.toolbar, text="▷", bg=COL_BTN_FACE, fg=COL_ACCENT,
                 font=("Arial", 12, "bold"), relief=tk.FLAT, padx=10,
                 command=self.run_current_frame).pack(side=tk.RIGHT, padx=5)
        tk.Label(self.toolbar, text="Frame", bg=COL_TOOLBAR, fg=COL_TEXT_DIM, font=FONT_SMALL).pack(side=tk.RIGHT)
          
        # Zoom controls
        zoom_frame = tk.Frame(self.toolbar, bg=COL_TOOLBAR)
        zoom_frame.pack(side=tk.RIGHT, padx=10)
        tk.Button(zoom_frame, text="-", bg=COL_BTN_FACE, fg=COL_BTN_TEXT, width=2,
    ...
    def setup_frame_editor(self):
        ...
        left_panel = tk.Frame(paned, bg=COL_PANEL, width=240)
        paned.add(left_panel, minsize=200)
          
        # Object tree header
        header_frame = tk.Frame(left_panel, bg=COL_PANEL_HEADER, height=24)
        header_frame.pack(fill=tk.X)
        tk.Label(header_frame, text="Workspace Toolbar", bg=COL_PANEL_HEADER, fg=COL_TEXT,
                 font=FONT_BOLD, anchor="w", padx=5).pack(fill=tk.BOTH, expand=True)
        ...
        style.configure("Dark.Treeview", 
                       background=COL_BG_LIGHT, 
                       foreground=COL_TEXT,
                       fieldbackground=COL_BG_LIGHT, 
                       borderwidth=0,
                       font=FONT_MAIN)
        ...
        right_panel = tk.Frame(paned, bg=COL_PANEL, width=280)
        paned.add(right_panel, minsize=220)
          
        # Properties Header
        prop_header = tk.Frame(right_panel, bg=COL_PANEL_HEADER, height=24)
        prop_header.pack(fill=tk.X)
        tk.Label(prop_header, text="Properties", bg=COL_PANEL_HEADER, fg=COL_TEXT,
                font=FONT_BOLD, anchor="w", padx=5).pack(fill=tk.BOTH, expand=True)
        ...
        prop_canvas.create_window((0, 0), window=self.prop_container, anchor="nw", width=260) # Set width to force resize
        ...
        # Ensure prop container resizes with canvas
        def on_canvas_configure(event):
            prop_canvas.itemconfig(prop_canvas.create_window((0,0), window=self.prop_container, anchor="nw"), width=event.width)
    ...
    def setup_storyboard(self):
        ...
        tk.Label(sb_toolbar, text="Storyboard Editor", bg=COL_TOOLBAR, fg=COL_ACCENT,
                font=FONT_BOLD).pack(side=tk.LEFT, padx=10)
    ...
    def setup_statusbar(self):
        self.statusbar = tk.Frame(self.root, bg=COL_ACCENT, height=24) # Fusion often uses blue status bar
        self.statusbar.pack(fill=tk.X, side=tk.BOTTOM)
        self.statusbar.pack_propagate(False)
          
        self.status_label = tk.Label(self.statusbar, text="Ready", bg=COL_ACCENT, fg="white",
                                    font=FONT_SMALL, anchor="w", padx=10)
        self.status_label.pack(side=tk.LEFT)
          
        self.pos_label = tk.Label(self.statusbar, text="X: 0  Y: 0", bg=COL_ACCENT, fg="white",
                                 font=FONT_SMALL, padx=10)
        self.pos_label.pack(side=tk.RIGHT)
          
        self.obj_count_label = tk.Label(self.statusbar, text="Objects: 0", bg=COL_ACCENT, fg="white",
                                       font=FONT_SMALL, padx=10)
        self.obj_count_label.pack(side=tk.RIGHT)
          
        self.frame_label = tk.Label(self.statusbar, text="Frame 1/1", bg=COL_ACCENT, fg="white",
                                   font=FONT_SMALL, padx=10)
    ...
    def switch_editor(self, name):
        ...
        for key, btn in self.tab_buttons.items():
            if key == name:
                btn.configure(bg=COL_BG_LIGHT, relief=tk.SUNKEN)
            else:
                btn.configure(bg=COL_BTN_FACE, relief=tk.FLAT)
    ...
    def refresh_canvas(self):
        ...
        # Frame border - Clickteam style dashed
        self.editor_canvas.create_rectangle(0, 0, fw*z, fh*z, outline="#000000", width=1,
                                           dash=(4, 4), tags="border")
    ...
    def draw_object(self, obj, z=1.0):
        ...
        sel_outline = COL_SELECTION if is_selected else outline
        sel_width = 2 if is_selected else 1
        ...
        if not obj.get("visible", True):
            ...
            self.editor_canvas.create_rectangle(x, y, x+w, y+h,
                                               outline="#888888", width=1, dash=(2, 2),
                                               tags=("object", tag))
        ...
        # Selection handles (Fusion style: small black squares)
        if is_selected:
            hs = 3 * z
            handle_col = "black"
            for hx, hy in [(x, y), (x+w, y), (x, y+h), (x+w, y+h),
                ...
                self.editor_canvas.create_rectangle(hx-hs, hy-hs, hx+hs, hy+hs,
                                                   fill=handle_col, outline="white",
                                                   tags=("handle", tag))
               
            # Object name tooltip
            self.editor_canvas.create_text(x, y - 10*z, text=obj.get("name", ""),
                                          fill=COL_ACCENT, font=FONT_SMALL,
    ...
    def edit_selected_object(self):
        ...
        def make_field(parent, label, key, var_type=tk.StringVar):
            ...
            entry = tk.Entry(frame, textvariable=var, bg=COL_BG_LIGHT, fg=COL_TEXT,
                            insertbackground=COL_TEXT, font=FONT_MAIN)
        ...
        tk.Checkbutton(content, text="Solid (collision)", variable=solid_var,
                      ...
                      activebackground=COL_BG, font=FONT_MAIN).pack(anchor="w", padx=10, pady=2)
    ...
    def refresh_properties(self):
        ...
        if not self.selected_object:
            tk.Label(self.prop_container, text="No Selection", bg=COL_PANEL, fg=COL_TEXT_DIM,
                    font=("Segoe UI", 10, "italic")).pack(pady=20)
            return
          
        obj = self.selected_object
          
        # Header - Fusion style block
        type_color = OBJ_COLORS.get(obj["type"], ("#888", "#666"))[0]
        header = tk.Frame(self.prop_container, bg=type_color, height=24)
        header.pack(fill=tk.X, pady=(0, 1))
        tk.Label(header, text=f" {obj['type']}", bg=type_color, fg="white",
                font=("Segoe UI", 9, "bold"), anchor="w").pack(fill=tk.X, padx=2)
        ...
        def prop_row(label, key, var_class=tk.StringVar, **kwargs):
            ...
            # Label Frame
            lbl_f = tk.Frame(row, bg=COL_BG_LIGHT, width=100)
            lbl_f.pack(side=tk.LEFT, fill=tk.Y)
            lbl_f.pack_propagate(False)
            
            tk.Label(lbl_f, text=label, bg=COL_BG_LIGHT, fg=COL_TEXT, 
                    font=FONT_MAIN, anchor="w", padx=5).pack(fill=tk.BOTH, expand=True)
            
            # Value Frame
            val_f = tk.Frame(row, bg=COL_PANEL)
            val_f.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
            
            var = var_class(value=obj.get(key, kwargs.get("default", "")))
            entry = tk.Entry(val_f, textvariable=var, bg=COL_PANEL, fg=COL_TEXT,
                            insertbackground=COL_TEXT, font=FONT_MAIN, relief=tk.FLAT)
            entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
            
            # Separator line
            tk.Frame(self.prop_container, bg="#444", height=1).pack(fill=tk.X)
        ...
        prop_row("Name", "name")
        prop_row("X Position", "x", tk.IntVar)
        prop_row("Y Position", "y", tk.IntVar)
        ...
        def info_row(label, value):
            ...
            tk.Label(lbl_f, text=label, bg=COL_BG_LIGHT, fg=COL_TEXT, 
                    font=FONT_MAIN, anchor="w", padx=5).pack(fill=tk.BOTH, expand=True)
            tk.Label(row, text=str(value), bg=COL_PANEL, fg=COL_BLUE_TEXT, 
                    font=FONT_MAIN, anchor="w", padx=2).pack(side=tk.LEFT, fill=tk.X, expand=True)
            tk.Frame(self.prop_container, bg="#444", height=1).pack(fill=tk.X)

        info_row("Movement", obj.get('movement', 'Static'))
        info_row("Layer", obj.get('layer', 0))
        info_row("Solid", 'Yes' if obj.get('solid') else 'No')
        ...
        tk.Button(self.prop_container, text="Edit Details...", bg=COL_BTN_FACE, fg=COL_BTN_TEXT,
                 relief=tk.FLAT, font=FONT_MAIN, command=self.edit_selected_object).pack(fill=tk.X, padx=2, pady=1)
        tk.Button(self.prop_container, text="Animation Editor", bg=COL_BTN_FACE, fg=COL_BTN_TEXT,
                 relief=tk.FLAT, font=FONT_MAIN,
                 command=lambda: AnimationEditor(self.root, self.selected_object)).pack(fill=tk.X, padx=2, pady=1)
        tk.Button(self.prop_container, text="Delete Object", bg=COL_DANGER, fg="white",
                 relief=tk.FLAT, font=FONT_MAIN, command=self.delete_selected_object).pack(fill=tk.X, padx=2, pady=1)
